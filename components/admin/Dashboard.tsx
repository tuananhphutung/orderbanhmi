...
const Dashboard: React.FC<DashboardProps> = ({ adminUser, users, orders, shifts }) => {
  ...
  const handleAIAnalyze = async () => {
    ...
    try {
        ...
        // Khởi tạo instance AI bằng Key từ environment
        const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
        
        const prompt = `Bạn là trợ lý ảo chuyên nghiệp cho cửa hàng Bánh Mì Hội An. 
        Dữ liệu bán hàng: ${JSON.stringify(contextData)}
        Câu hỏi: "${aiQuery}"
        
        Yêu cầu trả lời:
        1. Ngôn ngữ: Tiếng Việt, thân thiện.
        2. Phong cách: Cực kỳ ngắn gọn, dùng gạch đầu dòng nếu cần.
        3. Tập trung vào các con số và gợi ý kinh doanh.`;
        ...
        setAiResponse(response.text || "AI không trả về nội dung.");
    } catch (error: any) {
        console.error("AI Error:", error);
        setAiResponse(`⚠️ Lỗi kết nối AI: ${error.message || 'Vui lòng kiểm tra lại cấu hình'}`);
    } finally {
    ...
    <div className="p-4 md:p-8 pb-24 animate-in fade-in">
        ...
        <div className="relative z-10">
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-1.5 flex items-center border border-white/20">
                <input 
                    ...
                    placeholder="Hỏi AI: 'Hôm nay doanh thu thế nào?'"
                    className="flex-1 bg-transparent border-none outline-none text-white placeholder-indigo-200 px-4 py-2"
                />
                <button 
                    onClick={handleAIAnalyze}
                    disabled={isAnalyzing || !aiQuery.trim()}
                    className="p-3 bg-white text-indigo-600 rounded-xl hover:bg-indigo-50 shadow-lg transition-all active:scale-95 disabled:opacity-50"
                >
            ...
            {aiResponse && (
                <div className="mt-4 bg-black/20 backdrop-blur-sm rounded-2xl p-4 border border-white/10 animate-in slide-in-from-top-2 text-sm max-h-60 overflow-y-auto leading-relaxed">
                    {aiResponse}
                </div>
            )}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
          <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest">Doanh thu hôm nay</p>
          <p className="text-3xl font-black text-gray-800 mt-1">{totalRevenue.toLocaleString('vi-VN')} đ</p>
        </div>
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
          <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest">Lượng đơn hàng</p>
        ...
        <div className="bg-orange-500 p-6 rounded-3xl shadow-lg text-white">
          <p className="text-orange-100 text-[10px] font-bold uppercase tracking-widest">Món bán tốt nhất</p>
                    ...
                    {orders.slice(0, 10).map(order => (
                        <tr key={order.id} className="hover:bg-gray-50 transition-colors">
                            <td className="px-6 py-4">
                                <p className="font-bold text-gray-800">{new Date(order.timestamp).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}</p>
                                <p className="text-[10px] text-gray-400">ID: {order.id.slice(-4)}</p>
                            </td>
